"""
Shemagh Detection Experiment Grid — Find the best config for >95% shemagh detection.
Runs on H100 Kaggle. Evaluates against labels.txt ground truth (shemagh column).
...
parser = argparse.ArgumentParser(description='Shemagh Detection Experiment Grid')
...
WORK_DIR = "./yolo_shemagh_exp"
RESULTS_FILE = "shemagh_experiment_results.csv"
...
# ══════════════════════════════════════════════════════════════════════════════
# 1. Prepare Data — SHEMAGH (class 1)
# ══════════════════════════════════════════════════════════════════════════════
def prepare_shemagh_data(include_grayscale=False):
    """Prepare shemagh-only YOLO training data (class 1 → remapped to 0)."""
    import cv2
    ...
    # Filter: only images that have SHEMAGH annotations (class 1)
    shemagh_files = []
    for f in all_files:
            ...
            with open(lbl_path, 'r') as lf:
                for line in lf:
                    if line.strip().startswith('1 '):
                        shemagh_files.append(f)
                        break
    
    # Also include some negative examples (no shemagh) for better precision
    no_shemagh_files = [f for f in all_files if f not in shemagh_files]
    import random
    random.seed(42)
    neg_sample = random.sample(no_shemagh_files, min(len(no_shemagh_files), len(shemagh_files) // 2))
    
    all_train_files = shemagh_files + neg_sample
    ...
    for split, files in [('train', train_files), ('val', val_files)]:
        for f in files:
            ...
            # Copy label (shemagh only = class 1, remap to class 0 for single-class model)
            lbl_path = f"{lbl_dir}/{f.replace('.jpg', '.txt')}"
            ...
            if os.path.exists(lbl_path):
                with open(lbl_path, 'r') as lf:
                    ...
                    for line in lf:
                        if line.strip().startswith('1 '):
                            # Remap class 1 → class 0 (single-class model)
                            shemagh_lines.append('0 ' + line.strip()[2:] + '\n')
                with open(out_lbl, 'w') as of:
                    of.writelines(shemagh_lines)
            else:
    ...
    # data.yaml — single class: shemagh
...
"""
    ...
    print(f"  Data prepared: {len(train_files)} train, {len(val_files)} val" +
          f" ({len(shemagh_files)} shemagh + {len(neg_sample)} negatives)" +
          (" (+grayscale copies)" if include_grayscale else ""))
    return f"{work}/data.yaml"

# ══════════════════════════════════════════════════════════════════════════════
# 2. Load Ground Truth — SHEMAGH column
# ══════════════════════════════════════════════════════════════════════════════
def load_ground_truth():
    """Load labels.txt ground truth for shemagh detection (column index 2)."""
    gt = {}
    with open(LABELS_FILE, 'r') as f:
        for line in f:
            ...
            if line and not line.startswith('#'):
                parts = line.split(',')
                gt[parts[0]] = int(parts[2])  # shemagh column (index 2)
    return gt

# ══════════════════════════════════════════════════════════════════════════════
# 3. Evaluate Shemagh Detection
# ══════════════════════════════════════════════════════════════════════════════
def evaluate_shemagh_detection(model, test_dir, gt, imgsz, conf_thresholds=[0.01, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5]):
...
def main():
    ...
    print(f"Ground truth: {sum(gt.values())} shemaghs out of {len(gt)} images")
    ...
    for exp_name, model_name, imgsz, batch, extra in EXPERIMENTS:
        ...
        data_yaml = prepare_shemagh_data(include_grayscale=use_gray)
        ...
        if not os.path.exists(best_weights):
            ...
            search_paths = [
                f"./shemagh_experiments/{exp_name}/weights/best.pt",
                f"./runs/detect/shemagh_experiments/{exp_name}/weights/best.pt",
                f"./runs/shemagh_experiments/{exp_name}/weights/best.pt",
            ]
        ...
        if not os.path.exists(best_weights):
            ...
            import subprocess
            result = subprocess.run(['find', '.', '-path', f'*shemagh_experiments*{exp_name}*best.pt'], 
        ...
        # Evaluate
        stats, detections = evaluate_shemagh_detection(model_best, test_dir, gt, imgsz)
    ...
    # Final Summary
    print("\n" + "=" * 80)
    print("  FINAL RESULTS SUMMARY — SHEMAGH DETECTION")
    print("=" * 80)
